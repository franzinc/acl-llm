...
(defun insert-shortq-if (shortq item)
  (let ((item-shortq (shortq-item-pq shortq))
    ...
    (when (>= (funcall accessor item) (threshold-shortq-value shortq))
      (setf (shortq-item-pq shortq) (insert-shortq accessor item item-shortq))
      (when (> (length item-shortq) top) (setf (shortq-item-pq shortq) (subseq item-shortq 0 top)))
      ;;;(print-shortq)
      )
    item-shortq))

(defun insert-shortq (accessor item lst)
  (if (null lst)
      (list item)
    (if (>= (funcall accessor item) (funcall accessor (car lst)))
        (cons item lst) 
      (cons (car lst) (insert-shortq  accessor item (cdr lst))))))
